...
export class DateFlow {
  private readonly _date: Date;
  private readonly _dateFormat: DateFormatType | undefined;
  private readonly _locale: LocaleType | undefined;
  ...
  constructor(
    {
      ...
      dateFormat,
      locale,
    }: { date?: Date | number | string, dateFormat?: DateFormatType, locale?: LocaleType } = {}
  ) {
   ...
   */
  private _modifyDate(amount: number, unit: UnitType): DateFlow {
    ...
    switch(unit) {
      ...
      case 'millisecond': newDate.setMilliseconds(newDate.getMilliseconds() + amount); break;
    }
  ...
  format(options: { dateFormat?: DateFormatType, locale?: LocaleType }): string;
  format(param?: DateFormatType | { dateFormat?: DateFormatType, locale?: LocaleType }): string {
    let dateFormat: DateFormatType | undefined;
    ...
    if (effectiveLocale) {
      ...
      if (dateFormat) {
        ...
        if (dateFormat.includes('HH:mm:ss')) {
          options.hour = 'numeric'; options.minute = 'numeric'; options.second = 'numeric'; options.hour12 = false;
        } else if (dateFormat.includes('HH:mm')) {
          options.hour = 'numeric'; options.minute = 'numeric'; options.hour12 = false;
        } else if (dateFormat.includes('HH')) {
          options.hour = 'numeric'; options.hour12 = false;
        }
      } else {
    ...
    let result: DateFormatType | string = dateFormat;
    ...
    if (result.includes('HH')) { result = result.replace('HH', hour); }
    if (result.includes('mm')) { result = result.replace('mm', minute); }
    if (result.includes('ss')) { result = result.replace('ss', second); }
   ...
   */
  private getTimeZoneFromLocale(locale: LocaleType): string {
    switch (locale) {
      ...
      case 'en-US': default: return 'America/New_York';
    }
  }
   ...
   */
  private _getOtherTime(date: Date | DateFlow): number {
   ...
   */
  equal(date: Date | DateFlow, mode: 'short' | 'strict' = 'strict'): boolean {
    if (mode === 'strict') {
      return this._date.getTime() === this._getOtherTime(date);
    } else {
